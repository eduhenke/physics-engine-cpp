...
void Graphics::DrawVector(const Vec2& v, Color col)
{
	Vec2 c = Vec2(ScreenWidth / 2, ScreenHeight / 2);
...
void Graphics::DrawVector(Vec2 v, Vec2 c, Color col)
{
	...
	DrawTriangle(v, u, l , col);
	DrawLine(v, c, col);
}

void Graphics::PutPixel( int x,int y,Color c )
{
	if (x < 0 || x >= (int)ScreenWidth || y < 0 || y >= int(Graphics::ScreenHeight)) return;
	assert( x >= 0 );
